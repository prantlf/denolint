...
fn trailing_newlines(tag: &Tag) -> String {
  ...
  let num_newlines = match tag {
    ...
    List(_) | Item | FootnoteDefinition(_) | Table(_) | TableHead | TableRow | TableCell => 1,
...
impl MarkdownColorizer {
  ...
  fn handle_tag(&mut self, tag: &Tag, is_start: bool) -> Vec<Attribute> {
    ...
    match tag {
      ...
      Item => {
        if is_start {
          let list_kind = self.list.as_mut().expect("ListKind should be set, but not");
        ...
        out_line.replace_range(
          ...
          &match item.inner {
            TokenOrComment::Token(token) => match token {
              ...
              Token::Num { .. } | Token::BigInt { .. } => decorate(&line[range], Attribute::Yellow),
            ...
            TokenOrComment::Comment { .. } => decorate(&line[range], Attribute::Gray),
          },
        );
      }
